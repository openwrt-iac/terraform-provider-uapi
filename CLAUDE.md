...
- **A change to an attribute's Terraform *type shape* needs a `SchemaVersion` bump and a `StateUpgrader`.** List <-> string is not a diff, it is undecodable prior state: `plan` aborts naming a schema mismatch, not the attribute. Set `SchemaVersion` on the descriptor (the generator emits the schema `Version`) and add the migration to `internal/provider/state_upgrades.go`. Adding or removing an attribute does not need one. Because the provider stamped no version before 3.0.1, version-0 state is ambiguous, so upgraders normalize rather than convert and must be no-ops on the new shape. Cost of missing this: 3.0.0 could not read any 2.x state holding a `uapi_firewall_redirect`.
