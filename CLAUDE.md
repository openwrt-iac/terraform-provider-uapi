...
- **A type change that crosses the list/scalar boundary needs a `SchemaVersion` bump and a `StateUpgrader`.** That boundary is the test, not "the type changed": Terraform's passthrough decoder coerces between JSON scalars (a stored `true` reads back as `"true"`, `"64"` as a number), so bool->string and string->number need nothing, while list<->string makes prior state undecodable and `plan` aborts naming a schema mismatch rather than the attribute. Verify with `tfprotov6.RawState.Unmarshal` before deciding. Set `SchemaVersion` on the descriptor (the generator emits the schema `Version`) and add the migration to `internal/provider/state_upgrades.go`; adding or removing an attribute needs neither. Because nothing stamped a version before 3.0.1, version-0 state is ambiguous, so upgraders normalize rather than convert and must be no-ops on the new shape. Cost of missing this: 3.0.0 could not read any 2.x state holding a `uapi_firewall_redirect`.
